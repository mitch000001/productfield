...
export default React.createClass({
  ...
  render: function() {
    ...
    ForceFieldAnatomy.QUADRANTS.forEach(function(quadrant) {
      ...
      labels.push(<text className={`Labels-character Labels-${quadrant.name}`} x={x * GU} y={-y * GU + considerHeight * LABEL_HEIGHT} textAnchor={textAnchor}>{quadrant.name}</text>)
      ...
      labels.push(<text className={`Labels-context Labels-${quadrant.labels[0]}`} x={x * GU} y={-y * GU + considerHeight * LABEL_HEIGHT} textAnchor={textAnchor}>{quadrant.labels[0]}</text>)
      ...
      labels.push(<text className={`Labels-context Labels-${quadrant.labels[1]}`} x={x * GU} y={-y * GU + considerHeight * LABEL_HEIGHT} textAnchor={textAnchor}>{quadrant.labels[1]}</text>)

      x = 2.5 * quadrant.coefficient.x;
      y = 2.5 * quadrant.coefficient.y;
      considerHeight = quadrant.coefficient.y > 0 ? 0 : 0.5;
      textAnchor = 'middle';
      labels.push(<text className={`Labels-core Labels-${quadrant.labels[2]}`} x={x * GU} y={-y * GU + considerHeight * LABEL_HEIGHT} textAnchor={textAnchor}>{quadrant.labels[2]}</text>)
